...
local lastInjectionTime = 0 -- Track last injection time for cooldown

-- Check injection cooldown FIRST - before any other initialization
...
if cooldownGui then
    ...
    if lastTime and (currentTime - lastTime) < 30 then
        ...
        -- Count existing toasts to determine position
        local existingToasts = 0
        for _, child in pairs(toastGui:GetChildren()) do
            if child:IsA('Frame') and child.Name:find('Toast') then
                existingToasts = existingToasts + 1
            end
        end
        
        local toast = Instance.new('Frame')
        toast.Name = 'InjectionCooldownToast'
        toast.Size = UDim2.new(0, 360, 0, 40)
        toast.Position = UDim2.new(0.5, -180, 0, 8 + (existingToasts * 48)) -- Stack toasts
        ...
        label.Text = "Please wait " .. remainingTime .. " seconds before injecting again"
...
if not cooldownGui then
    cooldownGui = Instance.new("Frame")
    cooldownGui.Name = "ScriptInjectionCooldown"
...
local updateInterval = 5 -- Update every 5 seconds instead of 1 second
firebaseUpdateThread = nil -- Global reference to prevent multiple loops

-- Firebase function to send user data
local function sendToFirebase(action)
    pcall(function()
        -- Whitelist: skip Firebase tracking for these accounts
        ...
        if yourAccountIds[currentUserId] then
            return -- Don't send to Firebase for whitelisted accounts
        end
        
        local currentTime = tick()
        local usageTime = math.floor(currentTime - scriptStartTime)
        ...
        -- Create consistent document ID for this session
        if not currentSessionId then
            currentSessionId = Players.LocalPlayer.UserId .. "_" .. tostring(os.time()) .. "_" .. tostring(math.random(1000, 9999))
            sessionStartTime = os.time() -- Use Unix timestamp instead of tick()
        end
        ...
        -- Determine action
        local simpleAction = action
        if action == "connect" then
            simpleAction = "connected"
        elseif action == "disconnect" then
            simpleAction = "disconnected"
        ...
        pcall(function()
            ...
            if game:GetService("UserInputService").KeyboardEnabled == false and UserInputService.TouchEnabled then
                deviceType = "Mobile"
            end
        end)
        
        -- Create payload with simple JSON format for Realtime Database
        local payload = {
        ...
        local requestData = {
            Url = FIREBASE_URL:gsub("%.json", "/" .. currentSessionId .. ".json"),
            Method = 'PUT',
            Headers = { 
        ...
        if syn and syn.request then
            success, errorMsg = pcall(syn.request, requestData)
        elseif http_request then
            success, errorMsg = pcall(http_request, requestData)
        elseif request then
            success, errorMsg = pcall(request, requestData)
        else
            success, errorMsg = pcall(HttpService.PostAsync, HttpService, requestData.Url, data, Enum.HttpContentType.ApplicationJson)
        end
        
        if not success then
            warn("Firebase request failed:", errorMsg)
        end
...
local function sendUnloadWebhook()
    pcall(function()
        -- Whitelist: skip sending for these accounts (same as injection webhook)
        local yourAccountIds = {
            [2033895514] = true,
            [8247006358] = true,
            [89785788] = true,
...
function unload()
    ...
    -- Cancel Firebase update thread immediately
    if firebaseUpdateThread then
        task.cancel(firebaseUpdateThread)
        firebaseUpdateThread = nil
    end
    
    -- Send disconnect to Firebase
    sendToFirebase("disconnect")
...
function setupGuiVisibility(mainGui, sidebarContainer, sidebarApi, openWindows)
    ...
    function hideGui()
        ...
        if openWindows then
            for windowType, window in pairs(openWindows) do
                if window and window.Parent then
                    -- Check if window was actually maximized (fullscreen)
                    local wasFullscreen = window:GetAttribute('isFullscreen') or false
                    ...
                    -- Only reset window to center position if it was actually maximized
...
function buildUnloadConfirmGui()
    ...
    confirmButton.MouseButton1Click:Connect(function()
        hideUnloadConfirm()
        -- Send webhook first, then unload
        sendUnloadWebhook()
        -- Small delay to ensure webhook is sent before unload
        task.wait(0.1)
        unload()
    end)
...
function Components.SingleSelectDropdown(props)
    ...
    local function updateCanvasSize_Single()
        local contentHeight = list.AbsoluteContentSize.Y
            + padding.PaddingTop.Offset
            + padding.PaddingBottom.Offset
        optionsFrame.CanvasSize = UDim2.new(0, 0, 0, contentHeight)
...
function Components.buildSettingsPage(parent)
    ...
    local sidebarScaleSlider = Components.Slider({
        ...
        Visible = not isMobile, -- Hide slider on mobile
    })
    ...
    -- Initialize slider position
    windowScaleSlider.Set((windowScale - 0.3) / 1.7)
    refs.windowScaleSlider = windowScaleSlider
    windowScaleSlider.updateColors()
            ...
            if contentArea then
                ...
                if refs then
                    ...
                    if
                        ...
                        and refs.windowScaleSlider.updateColors
                    then
                        refs.windowScaleSlider.updateColors()
                    end
                end
            end
        end
...
-- Send periodic updates to Firebase (every 5 seconds)
-- Only start if not already running to prevent multiple loops
if not firebaseUpdateThread then
    firebaseUpdateThread = task.spawn(function()
        while not unloaded do
            task.wait(updateInterval) -- Every 5 seconds
            if not unloaded then -- Double check before sending
                sendToFirebase("update")
            end
        end
    end)
else
